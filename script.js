document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupFeaturedCase(caseData, scrollToView = false, isUserSelected = false) {
                ...
                <div class="featured-case-left" style="flex: 0 0 45%; width: 45%; background-color: #f0f7ff;">
                    <div class="p-4 md:p-6" style="background-color: #f0f7ff;">
                        ...
                        <div class="thumbnail-area featured-thumbnail mb-4">
                            <video id="featured-video" controls preload="metadata" ${(caseData.video.includes('dify') || caseData.video.includes('Genspark') || caseData.video.includes('skywork') || caseData.video.includes('Excel')) ? '' : 'muted'} class="w-full ${(caseData.video.includes('dify') || caseData.video.includes('Genspark') || caseData.video.includes('skywork') || caseData.video.includes('Excel')) ? '' : 'no-audio'}" style="height: auto; max-height: none;">
                                <source src="${caseData.video}" type="video/mp4">
                                Your browser does not support the video tag.
                            </video>
                            <div class="video-progress mt-2 bg-gray-200 rounded-full h-1.5">
        ...
        if (video && progressBar) {
            if (caseData.video.includes('dify') || caseData.video.includes('Genspark') || caseData.video.includes('skywork') || caseData.video.includes('Excel')) {
                video.muted = false;
            } else {
                video.muted = true;
                video.volume = 0;
